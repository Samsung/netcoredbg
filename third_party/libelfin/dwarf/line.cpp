...
struct line_table::impl
{
        ...
        ubyte line_range;
        ubyte opcode_base;
        std::vector<ubyte> standard_opcode_lengths;
        ...
        impl() : 
                program_offset(0), minimum_instruction_length(0), maximum_operations_per_instruction(0),
                default_is_stmt(false), line_base(0), line_range(0), opcode_base(0),
                last_file_name_end(0), file_names_complete(false)
        {};

        bool read_file_entry(cursor *cur, bool in_header);
};

line_table::line_table(const std::shared_ptr<section> &sec, section_offset offset,
...
line_table::file::file(std::string path, uint64_t mtime, uint64_t length)
...
line_table::iterator::iterator(const line_table *table, section_offset pos)
        : table(table), entry(), regs(), pos(pos)
{
        if (table) {
...
#pragma GCC diagnostic warning "-Wswitch-enum"
                ...
                case DW_LNS::const_add_pc:
                        uarg = (255u - m->opcode_base) / m->line_range;
                        goto advance_pc;
                ...
                case DW_LNE::lo_user...DW_LNE::hi_user:
                        // XXX Vendor extensions
                        throw std::runtime_error("vendor line number opcode " +
